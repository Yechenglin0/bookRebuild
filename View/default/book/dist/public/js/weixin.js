	...
	var myVue = new Vue({
	    ...
	    methods: {
	        onInfinite: function onInfinite(con) {
	            ...
	            if (this.searchChunk) {
	                ...
	                if (con) {
	                    ...
	                    fetch('./index.php?data=' + con).then(function (res) {
	                        if (res.ok) {
	                            res.text().then(function (response) {
	                                _this.searchCon = false;
	                            }).catch(function (err) {
	                                console.error(err);
	                            });
	                        }
	                    ...
	                    setTimeout(function () {
	                        var temp = [];
	                        for (var i = this.searchBooks.length; i <= this.searchBooks.length + 9; i++) {
	                            temp.push({
	                                name: j++,
	                                love: true,
	                                show: false, // 点击书名，弹出书的信息框标志
	                        ...
	                        this.searchBooks = this.searchBooks.concat(temp);
	                        this.searchBooks.length -= 1;
	                        this.$nextTick(function () {
	                            this.$broadcast('$InfiniteLoading:loaded');
	                        });
	                    }.bind(this), 1000);
	                } else {}
	            } else if (this.newbookChunk) {
	                /* 获取新书 */
	                setTimeout(function () {
	                    var temp = [];
	        ...
	        changeLoveState_search: function changeLoveState_search(index) {
	            this.searchBooks[index].love = !this.searchBooks[index].love;
	        },
	        ...
	        loadMore: function loadMore() {
	            ...
	            fetch('./index.php').then(function (res) {
	                if (res.ok) {
	                    res.text().then(function (response) {
	                        _this2.newbookDatas.push(response);
	                    });
	                }
	            });
	        },
